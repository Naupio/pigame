...
-export([enum_symbol_by_value_LoginStatus/1, enum_value_by_symbol_LoginStatus/1]).
...
%% enumerated types
-type 'LoginStatus'() :: 'FAILED' | 'SUCCEEDED' | 'NOTFOUND'.
-export_type(['LoginStatus'/0]).
...
e_enum_LoginStatus('FAILED', Bin, _TrUserData) ->
    <<Bin/binary, 0>>;
e_enum_LoginStatus('SUCCEEDED', Bin, _TrUserData) ->
    <<Bin/binary, 1>>;
e_enum_LoginStatus('NOTFOUND', Bin, _TrUserData) ->
    <<Bin/binary, 2>>;
e_enum_LoginStatus(V, Bin, _TrUserData) ->
    e_varint(V, Bin).
...
decode_msg_loginReq(Bin, TrUserData) ->
    dfp_read_field_def_loginReq(Bin,
                                0,
                                0,
                                id(<<>>, TrUserData),
                                TrUserData).
...
dfp_read_field_def_loginReq(Other, Z1, Z2, F@_1,
                            TrUserData) ->
    dg_read_field_def_loginReq(Other,
                               Z1,
                               Z2,
                               F@_1,
                               TrUserData).
...
dg_read_field_def_loginReq(<<1:1, X:7, Rest/binary>>, N,
                           Acc, F@_1, TrUserData)
    when N < 32 - 7 ->
    dg_read_field_def_loginReq(Rest,
                               N + 7,
                               X bsl N + Acc,
                               F@_1,
                               TrUserData);
    ...
    case Key of
        ...
        _ ->
            case Key band 7 of
                ...
                2 ->
                    skip_length_delimited_loginReq(Rest,
                                                   0,
                                                   0,
                                                   F@_1,
                                                   TrUserData);
                3 ->
                    skip_group_loginReq(Rest,
                                        Key bsr 3,
                                        0,
                                        F@_1,
                                        TrUserData);
...
d_field_loginReq_cookie(<<0:1, X:7, Rest/binary>>, N,
    ...
    dfp_read_field_def_loginReq(RestF,
                                0,
                                0,
                                NewFValue,
                                TrUserData).
...
skip_varint_loginReq(<<0:1, _:7, Rest/binary>>, Z1, Z2,
                     F@_1, TrUserData) ->
    dfp_read_field_def_loginReq(Rest,
                                Z1,
                                Z2,
                                F@_1,
                                TrUserData).
...
skip_length_delimited_loginReq(<<0:1, X:7,
    ...
    dfp_read_field_def_loginReq(Rest2,
                                0,
                                0,
                                F@_1,
                                TrUserData).
...
decode_msg_loginResp(Bin, TrUserData) ->
    dfp_read_field_def_loginResp(Bin,
                                 0,
                                 0,
                                 id('FAILED', TrUserData),
                                 id(0, TrUserData),
                                 TrUserData).

dfp_read_field_def_loginResp(<<8, Rest/binary>>, Z1, Z2,
                             F@_1, F@_2, TrUserData) ->
    d_field_loginResp_result(Rest,
                             Z1,
                             Z2,
                             F@_1,
                             F@_2,
...
dfp_read_field_def_loginResp(<<16, Rest/binary>>, Z1,
    ...
    d_field_loginResp_user_id(Rest,
                              Z1,
                              Z2,
                              F@_1,
                              F@_2,
...
dfp_read_field_def_loginResp(Other, Z1, Z2, F@_1, F@_2,
                             TrUserData) ->
    dg_read_field_def_loginResp(Other,
                                Z1,
                                Z2,
                                F@_1,
                                F@_2,
...
dg_read_field_def_loginResp(<<1:1, X:7, Rest/binary>>,
    ...
    when N < 32 - 7 ->
    dg_read_field_def_loginResp(Rest,
                                N + 7,
                                X bsl N + Acc,
                                F@_1,
                                F@_2,
    ...
    case Key of
        ...
        _ ->
            case Key band 7 of
                ...
                2 ->
                    skip_length_delimited_loginResp(Rest,
                                                    0,
                                                    0,
                                                    F@_1,
                                                    F@_2,
...
d_field_loginResp_result(<<0:1, X:7, Rest/binary>>, N,
    ...
    {NewFValue, RestF} = {id(d_enum_LoginStatus(begin
...
skip_varint_loginResp(<<0:1, _:7, Rest/binary>>, Z1, Z2,
                      F@_1, F@_2, TrUserData) ->
    dfp_read_field_def_loginResp(Rest,
                                 Z1,
                                 Z2,
                                 F@_1,
                                 F@_2,
...
skip_length_delimited_loginResp(<<0:1, X:7,
    ...
    Length = X bsl N + Acc,
    <<_:Length/binary, Rest2/binary>> = Rest,
    dfp_read_field_def_loginResp(Rest2,
                                 0,
                                 0,
                                 F@_1,
                                 F@_2,
...
decode_msg_helloReq(Bin, TrUserData) ->
    dfp_read_field_def_helloReq(Bin,
                                0,
                                0,
                                id(<<>>, TrUserData),
                                TrUserData).
...
dfp_read_field_def_helloReq(Other, Z1, Z2, F@_1,
                            TrUserData) ->
    dg_read_field_def_helloReq(Other,
                               Z1,
                               Z2,
                               F@_1,
                               TrUserData).
...
dg_read_field_def_helloReq(<<1:1, X:7, Rest/binary>>, N,
                           Acc, F@_1, TrUserData)
    when N < 32 - 7 ->
    dg_read_field_def_helloReq(Rest,
                               N + 7,
                               X bsl N + Acc,
                               F@_1,
                               TrUserData);
    ...
    case Key of
        ...
        _ ->
            case Key band 7 of
                ...
                2 ->
                    skip_length_delimited_helloReq(Rest,
                                                   0,
                                                   0,
                                                   F@_1,
                                                   TrUserData);
                3 ->
                    skip_group_helloReq(Rest,
                                        Key bsr 3,
                                        0,
                                        F@_1,
                                        TrUserData);
...
d_field_helloReq_msg(<<0:1, X:7, Rest/binary>>, N, Acc,
    ...
    dfp_read_field_def_helloReq(RestF,
                                0,
                                0,
                                NewFValue,
                                TrUserData).
...
skip_varint_helloReq(<<0:1, _:7, Rest/binary>>, Z1, Z2,
                     F@_1, TrUserData) ->
    dfp_read_field_def_helloReq(Rest,
                                Z1,
                                Z2,
                                F@_1,
                                TrUserData).
...
skip_length_delimited_helloReq(<<0:1, X:7,
    ...
    Length = X bsl N + Acc,
    <<_:Length/binary, Rest2/binary>> = Rest,
    dfp_read_field_def_helloReq(Rest2,
                                0,
                                0,
                                F@_1,
                                TrUserData).
...
decode_msg_worldResp(Bin, TrUserData) ->
    dfp_read_field_def_worldResp(Bin,
                                 0,
                                 0,
                                 id(<<>>, TrUserData),
                                 TrUserData).
...
dfp_read_field_def_worldResp(Other, Z1, Z2, F@_1,
                             TrUserData) ->
    dg_read_field_def_worldResp(Other,
                                Z1,
                                Z2,
                                F@_1,
                                TrUserData).
...
dg_read_field_def_worldResp(<<1:1, X:7, Rest/binary>>,
                            N, Acc, F@_1, TrUserData)
    when N < 32 - 7 ->
    dg_read_field_def_worldResp(Rest,
                                N + 7,
                                X bsl N + Acc,
                                F@_1,
                                TrUserData);
    ...
    case Key of
        ...
        _ ->
            case Key band 7 of
                ...
                2 ->
                    skip_length_delimited_worldResp(Rest,
                                                    0,
                                                    0,
                                                    F@_1,
                                                    TrUserData);
                3 ->
                    skip_group_worldResp(Rest,
                                         Key bsr 3,
                                         0,
                                         F@_1,
                                         TrUserData);
...
d_field_worldResp_msg(<<0:1, X:7, Rest/binary>>, N, Acc,
    ...
    dfp_read_field_def_worldResp(RestF,
                                 0,
                                 0,
                                 NewFValue,
                                 TrUserData).
...
skip_varint_worldResp(<<0:1, _:7, Rest/binary>>, Z1, Z2,
                      F@_1, TrUserData) ->
    dfp_read_field_def_worldResp(Rest,
                                 Z1,
                                 Z2,
                                 F@_1,
                                 TrUserData).
...
skip_length_delimited_worldResp(<<0:1, X:7,
    ...
    Length = X bsl N + Acc,
    <<_:Length/binary, Rest2/binary>> = Rest,
    dfp_read_field_def_worldResp(Rest2,
                                 0,
                                 0,
                                 F@_1,
                                 TrUserData).
...
dfp_read_field_def_heartbeatReq(Other, Z1, Z2,
                                TrUserData) ->
    dg_read_field_def_heartbeatReq(Other,
                                   Z1,
                                   Z2,
                                   TrUserData).
...
dg_read_field_def_heartbeatReq(<<1:1, X:7,
                               ...
                               N, Acc, TrUserData)
    when N < 32 - 7 ->
    dg_read_field_def_heartbeatReq(Rest,
                                   N + 7,
                                   X bsl N + Acc,
                                   TrUserData);
dg_read_field_def_heartbeatReq(<<0:1, X:7,
    ...
    case Key band 7 of
        ...
        2 ->
            skip_length_delimited_heartbeatReq(Rest,
                                               0,
                                               0,
                                               TrUserData);
        3 ->
...
skip_varint_heartbeatReq(<<0:1, _:7, Rest/binary>>, Z1,
                         Z2, TrUserData) ->
    dfp_read_field_def_heartbeatReq(Rest,
                                    Z1,
                                    Z2,
                                    TrUserData).
...
skip_length_delimited_heartbeatReq(<<0:1, X:7,
    ...
    Length = X bsl N + Acc,
    <<_:Length/binary, Rest2/binary>> = Rest,
    dfp_read_field_def_heartbeatReq(Rest2,
                                    0,
                                    0,
                                    TrUserData).
...
decode_msg_heartbeatResp(Bin, TrUserData) ->
    dfp_read_field_def_heartbeatResp(Bin,
                                     0,
                                     0,
                                     id(0, TrUserData),
                                     TrUserData).
...
dfp_read_field_def_heartbeatResp(<<8, Rest/binary>>, Z1,
                                 Z2, F@_1, TrUserData) ->
    d_field_heartbeatResp_unixtime(Rest,
                                   Z1,
                                   Z2,
                                   F@_1,
                                   TrUserData);
...
dfp_read_field_def_heartbeatResp(Other, Z1, Z2, F@_1,
                                 TrUserData) ->
    dg_read_field_def_heartbeatResp(Other,
                                    Z1,
                                    Z2,
                                    F@_1,
                                    TrUserData).
...
dg_read_field_def_heartbeatResp(<<1:1, X:7,
                                ...
                                N, Acc, F@_1, TrUserData)
    when N < 32 - 7 ->
    dg_read_field_def_heartbeatResp(Rest,
                                    N + 7,
                                    X bsl N + Acc,
                                    F@_1,
                                    TrUserData);
    ...
    case Key of
        ...
        _ ->
            case Key band 7 of
                ...
                2 ->
                    skip_length_delimited_heartbeatResp(Rest,
                                                        0,
                                                        0,
                                                        F@_1,
                                                        TrUserData);
                3 ->
                    skip_group_heartbeatResp(Rest,
                                             Key bsr 3,
                                             0,
                                             F@_1,
                                             TrUserData);
...
d_field_heartbeatResp_unixtime(<<0:1, X:7,
    ...
    {NewFValue, RestF} = {id(X bsl N + Acc, TrUserData),
                          Rest},
    dfp_read_field_def_heartbeatResp(RestF,
                                     0,
                                     0,
                                     NewFValue,
                                     TrUserData).
...
skip_varint_heartbeatResp(<<1:1, _:7, Rest/binary>>, Z1,
                          Z2, F@_1, TrUserData) ->
    skip_varint_heartbeatResp(Rest,
                              Z1,
                              Z2,
                              F@_1,
                              TrUserData);
skip_varint_heartbeatResp(<<0:1, _:7, Rest/binary>>, Z1,
                          Z2, F@_1, TrUserData) ->
    dfp_read_field_def_heartbeatResp(Rest,
                                     Z1,
                                     Z2,
                                     F@_1,
                                     TrUserData).
...
skip_length_delimited_heartbeatResp(<<0:1, X:7,
    ...
    Length = X bsl N + Acc,
    <<_:Length/binary, Rest2/binary>> = Rest,
    dfp_read_field_def_heartbeatResp(Rest2,
                                     0,
                                     0,
                                     F@_1,
                                     TrUserData).
...
d_enum_LoginStatus(0) -> 'FAILED';
d_enum_LoginStatus(1) -> 'SUCCEEDED';
d_enum_LoginStatus(2) -> 'NOTFOUND';
d_enum_LoginStatus(V) -> V.
...
v_msg_loginResp(#loginResp{result = F1, user_id = F2},
    ...
    if F1 == undefined -> ok;
       true ->
           v_enum_LoginStatus(F1, [result | Path], TrUserData)
    end,
    if F2 == undefined -> ok;
...
v_msg_heartbeatResp(#heartbeatResp{unixtime = F1}, Path,
                    TrUserData) ->
    if F1 == undefined -> ok;
       true -> v_type_uint64(F1, [unixtime | Path], TrUserData)
    end,
    ok;
v_msg_heartbeatResp(X, Path, _TrUserData) ->
    mk_type_error({expected_msg, heartbeatResp}, X, Path).

-compile({nowarn_unused_function,v_enum_LoginStatus/3}).
-dialyzer({nowarn_function,v_enum_LoginStatus/3}).
v_enum_LoginStatus('FAILED', _Path, _TrUserData) -> ok;
v_enum_LoginStatus('SUCCEEDED', _Path, _TrUserData) ->
    ok;
v_enum_LoginStatus('NOTFOUND', _Path, _TrUserData) ->
    ok;
v_enum_LoginStatus(V, Path, TrUserData)
    when is_integer(V) ->
    v_type_sint32(V, Path, TrUserData);
v_enum_LoginStatus(X, Path, _TrUserData) ->
    mk_type_error({invalid_enum, 'LoginStatus'}, X, Path).
    ...
    [{{enum, 'LoginStatus'},
     ...
     {{msg, loginResp},
      [#field{name = result, fnum = 1, rnum = 2,
              type = {enum, 'LoginStatus'}, occurrence = optional,
              opts = []},
...
find_enum_def('LoginStatus') ->
    [{'FAILED', 0}, {'SUCCEEDED', 1}, {'NOTFOUND', 2}];
find_enum_def(_) -> error.


enum_symbol_by_value('LoginStatus', Value) ->
    enum_symbol_by_value_LoginStatus(Value).


enum_value_by_symbol('LoginStatus', Sym) ->
    enum_value_by_symbol_LoginStatus(Sym).


enum_symbol_by_value_LoginStatus(0) -> 'FAILED';
enum_symbol_by_value_LoginStatus(1) -> 'SUCCEEDED';
enum_symbol_by_value_LoginStatus(2) -> 'NOTFOUND'.


enum_value_by_symbol_LoginStatus('FAILED') -> 0;
enum_value_by_symbol_LoginStatus('SUCCEEDED') -> 1;
enum_value_by_symbol_LoginStatus('NOTFOUND') -> 2.
...
fqbin_to_enum_name(<<"common.proto.LoginStatus">>) -> 'LoginStatus';
fqbin_to_enum_name(E) ->
    error({gpb_error, {badenum, E}}).


enum_name_to_fqbin('LoginStatus') -> <<"common.proto.LoginStatus">>;
...
get_enum_containment("common") -> ['LoginStatus'];
...
get_proto_by_enum_name_as_fqbin(<<"common.proto.LoginStatus">>) -> "common";
